...
REM    This batch file generates, (or uses an existing,)
REM    ooShapes.ver.incl file.
REM
...
REM    not a working directory, it checks for an existing
REM    ooShapes.ver.incl file, which may have been included in a source
REM    file package when the package was created.  If not a svn directory, and
REM    no ooShapes.ver.incl file, simply copy ooShapes.ver to
REM    ooShapes.ver.incl.
REM - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

pushd src
cd

REM  First parse ooShapes.ver to get the existing version numbers.
for /F "eol=# delims== tokens=1,2,3*" %%i in (ooShapes.ver) do (
 if %%i == OOSHAPES_MAJOR set MAJOR_NUM=%%j
 if %%i == OOSHAPES_MINOR set MINOR_NUM=%%j
 if %%i == OOSHAPES_MOD_LVL set LVL_NUM=%%j
 if %%i == OOSHAPES_BLD_LVL set BLD_NUM=%%j
)
...
REM  Now write out ooShapes.ver.incl
if exist ooShapes.ver.incl del /F /Q ooShapes.ver.incl
for /F "delims== tokens=1,2,3*" %%i in (ooShapes.ver) do (
 if %%i == OOSHAPES_BLD_LVL (
   echo %%i=%SVN_REV%>> ooShapes.ver.incl
   set BLD_NUM=%SVN_REV%
 ) else (
   if %%i == OOSHAPES_VER_STR (
     echo %%i="%MAJOR_NUM%.%MINOR_NUM%.%LVL_NUM%.%SVN_REV%">> ooShapes.ver.incl
   ) else (
     if %%jx == x (
       echo %%i>> ooShapes.ver.incl
     ) else (
       echo %%i=%%j>> ooShapes.ver.incl
     )
   )
 )
)
echo SVN_REVSION=%SVN_REV%>> ooShapes.ver.incl
goto DONE_OK

:NOSVN
if NOT exist ooShapes.ver.incl (
  for /F "delims== tokens=1,2,3*" %%i in (ooShapes.ver) do (
    if %%i == OOSHAPES_BLD_LVL (
      echo %%i=%%j>> ooShapes.ver.incl
    ) else (
      if %%i == OOSHAPES_VER_STR (
        echo %%i="%MAJOR_NUM%.%MINOR_NUM%.%LVL_NUM%.%BLD_NUM%">> ooShapes.ver.incl
      ) else (
        if %%jx == x (
          echo %%i>> ooShapes.ver.incl
        ) else (
  ...
  echo SVN_REVSION=0 >> ooShapes.ver.incl
)
