/* This is the supporting header file for WIN64 threads.
 * Checked with MSVC 7.0.
 ...
 *      external (rexxsaa.h).
 *      It should return (RxPackageGlobalDataDef *) of the current thread.
 * 7) __rxpack_get_tsd()
 *    = pointer to a variable of type RxPackageGlobalDataDef.
...
# define THREAD_PROTECT(varname)   __tryVK( myHandler ) RxPackSetMutex(&varname);
# define THREAD_UNPROTECT(varname) __exceptVK           RxPackUnsetMutex(&varname);
#else
# define THREAD_PROTECT(varname) __try { RxPackSetMutex(&varname);
# define THREAD_UNPROTECT(varname) } __finally { RxPackUnsetMutex(&varname); }
#endif
...
#define GLOBAL_ENTRY_POINT() RxPackInitializeThread()

unsigned long RxPackGetThreadID(void);

/* we need a pointer to RxPackageGlobalDataDef sometimes: */
