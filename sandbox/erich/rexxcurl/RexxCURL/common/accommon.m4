...
case "$with_rexx" in
   regina)               dnl -------- Regina
      ...
      if test "$ac_cv_prog_regina_config" = yes; then
         ...
         AX_COMPARE_VERSION([$REXX_VER],[gt],[3.6],[REXX_ADDONS=`regina-config --addons`],[REXXADDONS=$libdir])
         REXX_VER=`regina-config --version`
      else
         case "$target" in
            *nto-qnx*)
...
mh_sysv_incdir=""
mh_sysv_libdir=""
all_words="clang gcc xlc acc cc"
ac_dir=""
...
# otherwise you will get memory corruption.
      LD_RXLIB1="${CC}  ${LDFLAGS} ${EEXTRA} -dynamiclib -install_name=\$(@) -headerpad_max_install_names"
      LD_RXTRANSLIB1="$LD_RXLIB1"
      ;;
   *)
      ;;
