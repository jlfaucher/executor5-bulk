...
REM - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
REM  setOOSQLiteEnv.bat
REM    For developers (or early adopters) this batch file can be useful to set
REM    the environment for testing ooSQLite programs.
REM
REM    It assumes that ooSQLite3.exe, ooSQLite.cls, and ooSQLite.dll are in
REM    the bin subdirectory of this directory.  It then adds that directory to
REM    the path so that the files can be found by test programs.
REM
REM    For early adopters, if you are not going to be rebuilding any of
REM    ooSQLite3.exe, ooSQLite.cls, or oosqlite.dll, it is probably easier to
