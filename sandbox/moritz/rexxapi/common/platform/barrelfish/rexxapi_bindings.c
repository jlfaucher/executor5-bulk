...
static void error_handler(struct rexxapi_binding *_binding, errval_t err) {
	DEBUG_ERR(err, "binding error");
}

static int in_dispatch = 0;

static errval_t my_event_dispatch(struct waitset *ws) {
...
//	debug_printf("%p {\n", __builtin_return_address(0));
	err = my_event_dispatch(ws);
//	debug_printf("}\n");
	in_dispatch--;
	if (err_is_fail(err)) {
		USER_PANIC_ERR(err, "%s: failed waiting for message.", __FUNCTION__);
	}
}
	...
	*actual = 0;
	while (*actual < expected) {
		...
		if (remaining <= expected) {
			...
			memcpy(buffer + *actual, message->buffer, remaining);
			*actual += remaining;
			free(message->buffer);
			free(message);
		} else {
			memcpy(buffer + *actual, message->buffer, expected);
			*actual += expected;
...
//	debug_printf("%s:%s:%d sending %zx\n", __FILE__, __FUNCTION__, __LINE__, connection->pending_message.size);
...
int rexx_write(struct rexx_connection *connection, uint8_t *buffer,
		size_t expected, size_t *actual) {
//	debug_printf("%s:%s:%d buffer=%p expected=%zx\n", __FILE__, __FUNCTION__,
//	__LINE__, buffer, expected);

	*actual = 0;

	while (*actual < expected) {

		size_t diff = expected - *actual;
		if (diff > MAX_MESSAGE_SIZE) {
			diff = MAX_MESSAGE_SIZE;
		}

		volatile struct message *msg = &connection->pending_message;
		msg->buffer = buffer + *actual;
		msg->size = diff;
		msg->offset = 0;
...
//	debug_printf("%s:%s:%d \n", __FILE__, __FUNCTION__, __LINE__);
	...
	struct rexx_connection *connection = calloc(1,
			sizeof(struct rexx_connection));
...
//			__FUNCTION__, __LINE__, message->size, message->offset);
...
int rexx_server_read(struct rexx_client *client, uint8_t *buffer,
		size_t expected, size_t *actual) {
//	debug_printf("%s:%s:%d \n", __FILE__, __FUNCTION__, __LINE__);
//	debug_printf("client=%p client->messages=%p buffer=%p expected=%zx\n",
...
//	debug_printf("%s:%s:%d \n", __FILE__, __FUNCTION__, __LINE__);
	...
	if (err_is_fail(err)) {
		...
		if (err_no(err) == FLOUNDER_ERR_TX_BUSY) {
			...
			err = client->binding->register_send(client->binding, ws, txcont);
			if (err_is_fail(err)) {
				// note that only one continuation may be registered at a time
