...
<!-- define variable normally passed by Publican -->
<xsl:variable name="clean_title">Open Object Rexx</xsl:variable>
...
<xsl:param name="ignore.image.scaling" select="1"/> <!-- changed to 1 since all images are now SVG -->
...
<xsl:param name="tablecolumns.extension">0</xsl:param> <!-- changed to 0 since we are not using Perl -->
...
<xsl:template name="head.content">
    ...
    <meta xmlns="http://www.w3.org/1999/xhtml" name="generator">
        <xsl:attribute name="content">  <!-- replace next line to remove publican references
            <xsl:text>publican </xsl:text><xsl:value-of select="$publican.version"/>    -->
            <xsl:text>xsltproc </xsl:text><xsl:value-of select="$xsltproc.version"/>
        </xsl:attribute>
    </meta>
