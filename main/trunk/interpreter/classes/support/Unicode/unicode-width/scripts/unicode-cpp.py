...
MODULE_PATH = "../src-cpp/gen/tables.cpp"
"""The path of the emitted C++ tables module (relative to the working directory)"""

MODULE_HEADER_PATH = "../src-cpp/gen/tables.hpp"
"""The path of the emitted C++ tables header (relative to the working directory)"""

PROPS_PATH = "../src-cpp/gen/props.cpp"
"""The path of the emitted generated props (relative to the working directory)"""

PROPS_HEADER_PATH = "../src-cpp/gen/props.hpp"
"""The path of the emitted generated props header (relative to the working directory)"""

LOOKUP_PATH = "../src-cpp/gen/lookup.cpp"
"""The path of the emitted generated lookup functions (relative to the working directory)"""

LOOKUP_HEADER_PATH = "../src-cpp/gen/lookup.hpp"
"""The path of the emitted generated lookup functions header (relative to the working directory)"""

WIDTH_INFO_PATH = "../src-cpp/gen/width_info.cpp"
"""The path of the emitted WidthInfo constants (relative to the working directory)"""

WIDTH_INFO_HEADER_PATH = "../src-cpp/gen/width_info.hpp"
"""The path of the emitted WidthInfo constants header (relative to the working directory)"""

TEST_PATH = "../src-cpp/gen/tables_test.cpp"
"""The path of the emitted C++ tests (relative to the working directory)"""
...
FILE_HEADER = """// Ported from Rust to C++ by Claude Sonnet 5.
...
def emit_cpp_file(path: str, generator: Callable[[IO[str]], None]):
    """Outputs a C++ file to `path` using the provided `generator` function.
...
def lookup_fns(
    ...
    """Emits the C++ multi-level-trie walk (lookup_width / lookup_width_cjk).
    Structurally identical to the Rust version: same three-level table shape,
    same bit-shift arithmetic (kept in lockstep with TABLE_SPLITS), same
    "packed width < 3 is a direct answer, else consult the special-range
    match" fallback. Only the surface syntax (Rust `match` -> C++ `switch`+
    range checks) differs.
    """
    if is_cjk:
        cjk_lo = "_cjk"
        cjk_cap = "_CJK"
        ...
        cjk_lo = ""
        cjk_cap = ""
    s = f"""// Returns the UAX #11 based width of `c` by consulting a multi-level
// lookup table.
//
...
std::pair<std::uint8_t, WidthInfo> lookup_width{cjk_lo}(char32_t c) {{
    std::uint32_t cp = (std::uint32_t)c;

    std::uint8_t t1_offset = WIDTH_ROOT{cjk_cap}[cp >> {TABLE_SPLITS[1]}];
    ...
    std::uint8_t t2_offset = WIDTH_MIDDLE[t1_offset][cp >> {TABLE_SPLITS[0]} & 0x{(2 ** (TABLE_SPLITS[1] - TABLE_SPLITS[0]) - 1):X}];
    ...
    std::uint8_t packed_widths = WIDTH_LEAVES[t2_offset][cp >> 2 & 0x{(2 ** (TABLE_SPLITS[0] - 2) - 1):X}];
...
"""
    ...
    for (lo, hi), width in special_ranges:
        if width.is_carried():
            ...
            width_info = "DEFAULT"
        if hi == lo:
            s += f"    if (0x{lo:X} == c) return {{{width.width_alone()}, WidthInfo::{width_info}}};\n"
        else:
            s += f"    if (0x{lo:X} <= c && c <= 0x{hi:X}) return {{{width.width_alone()}, WidthInfo::{width_info}}};\n"

    s += """    return {2, WidthInfo::EMOJI_PRESENTATION};
}
"""
    return s
...
def emit_width_info(module: IO[str]):
    """Outputs the WidthInfo autogenerated constants, as static constexpr
    WidthInfo members. Ported 1:1 from the Rust `impl WidthInfo` constant
    block: same packed-u16 bit values, so `width_info.hpp`'s hand-written
    bitwise get/set methods (a faithful transcription of src/width_info.rs)
    operate identically to the Rust crate.
    """
    module.write(
...
"""
    ...
    for variant in WidthState:
        if variant.is_carried():
            module.write(
                f"const WidthInfo WidthInfo::{variant.name}{{0b{variant.value:016b}}};\n"
            )
...
def emit_props(
    ...
    """Outputs a C++ module to `module` containing generated property functions.
    Range-disjunction checks replace Rust's `matches!`; the two-level index +
    binary-search leaf lookups are otherwise structurally identical to the
    Rust version, just spelled in C++.
    """
    module.write('#include "../props.hpp"\n')
    module.write('#include "tables.hpp"\n')
    module.write("#include <algorithm>\n\n")
    module.write("namespace unicode_width {\n\n")

    def emit_range_disjunction(fn_name: str, doc: str, ranges: list[tuple[int, int]]):
        module.write(f"// {doc}\n")
        module.write(f"bool {fn_name}(char32_t c) {{\n")
        module.write("    return")
        if not ranges:
            module.write(" false;\n")
        else:
            for i, (lo, hi) in enumerate(ranges):
                ...
                op = "||" if i > 0 else "  "
                if hi == lo:
                    module.write(f"{sep}{op} c == 0x{lo:X}")
                else:
                    module.write(f"{sep}{op} (c >= 0x{lo:X} && c <= 0x{hi:X})")
            module.write(";\n")
        module.write("}\n\n")

    emit_range_disjunction(
        "is_joining_group_lam",
...
"""
    ...
    for msbs, i in emoji_presentation_idx:
        module.write(f"        case 0x{msbs:X}: idx_of_leaf = {i}; break;\n")
    module.write(
...
"""
    ...
    for msbs, i in text_presentation_idx:
        module.write(
            f"        case 0x{msbs:X}: leaf = TEXT_PRESENTATION_LEAF_{i}; "
            f"leaf_len = {len(text_presentation_leaves[i])}; break;\n"
        )
    module.write(
        """        default: return false;
    }
...
"""
    ...
    for msbs, i in emoji_modifier_idx:
        module.write(
            f"        case 0x{msbs:X}: leaf = EMOJI_MODIFIER_LEAF_{i}; "
            f"leaf_len = {len(emoji_modifier_leaves[i])}; break;\n"
        )
    module.write(
        """        default: return false;
    }
...
def emit_lookup(
    ...
    """Outputs a C++ module (to `module`) containing generated lookup
    functions, plus their declarations (to `header`) - lookup_width() and
    lookup_width_cjk() are called from the hand-written src-cpp/lookup.cpp,
    so they need a header, not just a source file.
    """
    header.write('#include <cstdint>\n#include <utility>\n#include "../width_info.hpp"\n\n')
    header.write("namespace unicode_width {\n\n")
    header.write(
    ...
    module.write('#include "lookup.hpp"\n')
    module.write('#include "tables.hpp"\n')
    module.write('#include "../width_info.hpp"\n\n')
    module.write("namespace unicode_width {\n\n")
...
def emit_tables(
    ...
    """Outputs C++ table data (to `module`) and matching extern declarations
    (to `header`), from the same `tables` data structure the Rust emitter
    uses. If `TABLE_CFGS` is edited, you must also edit lookup_fns() (in the
    hand-shaped lookup_width/lookup_width_cjk emission) to match.

    Unlike the Rust crate, there is no separate "cjk" Cargo feature here:
    both lookup_width() and lookup_width_cjk() are always compiled (that's
    what the template<bool IS_CJK> split needs), so every table below is
    emitted in full and unconditionally - the `#[cfg(feature = "cjk")]`
    splits in the Rust source simply disappear.
    """
    ...
    header.write(
        f"""// The version of Unicode (http://www.unicode.org/) that this table data
// is based on.
constexpr std::uint8_t UNICODE_VERSION[3] = {{ {unicode_version[0]}, {unicode_version[1]}, {unicode_version[2]} }};
    ...
    for i, table in enumerate(tables):
        ...
        if table.bytes_per_row is None:
            header.write(
                ...
                f"extern const std::uint8_t {table.name}[{len(byte_array)}];\n\n"
            )
            module.write(f"const std::uint8_t {table.name}[{len(byte_array)}] = {{")
            ...
            header.write(
                f"""constexpr std::size_t {table.name}_LEN = {num_rows};
...
extern const std::uint8_t {table.name}[{table.name}_LEN][{table.bytes_per_row}];

"""
            )
            module.write(
                f"const std::uint8_t {table.name}[{table.name}_LEN][{table.bytes_per_row}] = {{\n"
            )
    ...
    header.write(
        f"""// A closed codepoint range [lo, hi].
...
constexpr std::size_t NON_TRANSPARENT_ZERO_WIDTHS_LEN = {len(non_transparent_zero_widths)};
...
constexpr std::size_t SOLIDUS_TRANSPARENT_LEN = {len(solidus_transparent)};
...
"""
    ...
    module.write(
        f"const CodepointRange NON_TRANSPARENT_ZERO_WIDTHS[NON_TRANSPARENT_ZERO_WIDTHS_LEN] = {{\n"
    )
    for lo, hi in non_transparent_zero_widths:
        module.write(f"    {{ 0x{lo:X}, 0x{hi:X} }},\n")
    module.write("};\n\n")

    module.write(f"const CodepointRange SOLIDUS_TRANSPARENT[SOLIDUS_TRANSPARENT_LEN] = {{\n")
    ...
    header.write(
        f"""// Array of 1024-bit bitmaps. Index into the correct bitmap with the 10 LSB
// of your codepoint to get whether it can start an emoji presentation
// sequence.
extern const std::uint8_t EMOJI_PRESENTATION_LEAVES[{len(emoji_presentation_leaves)}][128];

"""
    )
    module.write(
        f"const std::uint8_t EMOJI_PRESENTATION_LEAVES[{len(emoji_presentation_leaves)}][128] = {{\n"
    )
    ...
    # text presentation table

    for leaf_idx, leaf in enumerate(text_presentation_leaves):
        header.write(
            f"extern const RangePair TEXT_PRESENTATION_LEAF_{leaf_idx}[{len(leaf)}];\n"
        )
        module.write(
            f"const RangePair TEXT_PRESENTATION_LEAF_{leaf_idx}[{len(leaf)}] = {{\n"
        )
        for lo, hi in leaf:
    ...
    for leaf_idx, leaf in enumerate(emoji_modifier_leaves):
        header.write(
            f"extern const RangePair EMOJI_MODIFIER_LEAF_{leaf_idx}[{len(leaf)}];\n"
        )
        module.write(
            f"const RangePair EMOJI_MODIFIER_LEAF_{leaf_idx}[{len(leaf)}] = {{\n"
        )
        for lo, hi in leaf:
            module.write(f"    {{ 0x{lo:02X}, 0x{hi:02X} }},\n")
        module.write("};\n\n")
...
def emit_tests(
    ...
    """Outputs a C++ module to `module` containing test data for character
    width. Both the non-CJK and CJK WidthInfo variant lists are always
    emitted (no feature gate), matching the rest of this port.
    """
    module.write('#include "../width_info.hpp"\n\n')
    module.write("namespace unicode_width {\n\n")
    ...
    module.write(
        f"const WidthInfo NORMALIZATION_TEST_WIDTHS[{len(test_width_variants) + 1}] = {{\n"
        f"    WidthInfo::DEFAULT,\n"
    )
    for variant in WidthState:
    ...
    module.write(
        f"const WidthInfo NORMALIZATION_TEST_WIDTHS_CJK[{len(test_width_variants_cjk) + 1}] = {{\n"
        f"    WidthInfo::DEFAULT,\n"
    )
    for variant in WidthState:
    ...
    module.write(
        f"struct NormalizationTestCase {{\n"
        f"    const char *orig, *nfc, *nfd, *nfkc, *nfkd;\n"
        f"}};\n\n"
        f"const NormalizationTestCase NORMALIZATION_TEST[{len(normalization_tests)}] = {{\n"
    )
    for orig, nfc, nfd, nfkc, nfkd in normalization_tests:
        def cpp_str(s: str) -> str:
            # C++ has no Rust-style raw string literal that's always safe for
            # arbitrary Unicode text with embedded quotes; escape instead.
            return '"' + s.replace("\\", "\\\\").replace('"', '\\"') + '"'

        module.write(
            f"    {{ {cpp_str(orig)}, {cpp_str(nfc)}, {cpp_str(nfd)}, {cpp_str(nfkc)}, {cpp_str(nfkd)} }},\n"
        )
    module.write("};\n\n} // namespace unicode_width\n")
...
def main(module_path: str):
    ...
    # emit_tables writes both tables.cpp (definitions) and tables.hpp
    # (extern declarations) from the same pass, so it doesn't go through
    # emit_cpp_file like the others.
    if os.path.exists(MODULE_PATH):
        os.remove(MODULE_PATH)
    if os.path.exists(MODULE_HEADER_PATH):
        os.remove(MODULE_HEADER_PATH)
    os.makedirs(os.path.dirname(MODULE_PATH), exist_ok=True)
    with open(MODULE_PATH, "w", newline="\n", encoding="utf-8") as f_cpp, open(
        MODULE_HEADER_PATH, "w", newline="\n", encoding="utf-8"
    ) as f_hpp:
        f_cpp.write(FILE_HEADER)
        f_hpp.write(FILE_HEADER)
    ...
    print(f'Wrote to "{MODULE_PATH}" and "{MODULE_HEADER_PATH}"')
    ...
    if os.path.exists(LOOKUP_HEADER_PATH):
        os.remove(LOOKUP_HEADER_PATH)
    os.makedirs(os.path.dirname(LOOKUP_PATH), exist_ok=True)
    with open(LOOKUP_PATH, "w", newline="\n", encoding="utf-8") as f_cpp, open(
        LOOKUP_HEADER_PATH, "w", newline="\n", encoding="utf-8"
    ) as f_hpp:
        f_cpp.write(FILE_HEADER)
        f_hpp.write(FILE_HEADER)
        f_hpp.write("#pragma once\n")
        emit_lookup(f_cpp, f_hpp, special_ranges, cjk_special_ranges, joining_group_lam)
    print(f'Wrote to "{LOOKUP_PATH}" and "{LOOKUP_HEADER_PATH}"')

    emit_cpp_file(WIDTH_INFO_PATH, emit_width_info)
    print(f'Wrote to "{WIDTH_INFO_PATH}"')

    emit_cpp_file(TEST_PATH, lambda f: emit_tests(f, normalization_tests))
